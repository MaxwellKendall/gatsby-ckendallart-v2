        ...
        <Layout>
            <form ref={form} className="flex-col-center w-full justify-center">
                <div className="flex flex-wrap w-full text-center justify-center">
                    ...
                    <input name="name" type="text" value={values.name} onChange={(e) => handleUpdate(e, 'name')} />
                    <label className="mr-5">Phone Number:</label>
                    <input name="phone" type="tel" value={values.phone} onChange={(e) => handleUpdate(e, 'phone')} />
                    <label className="mr-5">Email:</label>
                    <input name="email" type="email" value={values.email} onChange={(e) => handleUpdate(e, 'email')} />
                    <label className="mr-5">Contact Preference:</label>
                    <select id="canvas" name="preferredContact" value={values.preferredContact} onChange={(e) => handleUpdate(e, 'preferredContact')}>
                            <option value="email">Email</option>
                            <option value="phone">Phone</option>
                    </select>
                </div>
                <div className="flex flex-wrap w-full justify-center">
                    <legend className="w-full my-5 text-center">Details on your Piece:</legend>
                    <label className="w-full text-center">Example Image:</label>
                    <input ref={file} name="attachment" type="file" />
                    <label className="w-full mt-5 text-center">Canvas Size and Type</label>
                    <select name="" id="canvas" name="canvas" value={values.canvas} onChange={(e) => handleUpdate(e, 'canvas')}>
              ...
              <textarea name="details" id="" cols="50" rows="10" maxLength="500" value={values.details} onChange={(e) => handleUpdate(e, 'details')} />
            </form>
            <button
                disabled={isDisabled}
                onClick={submit}
                className={`${isDisabled ? 'cursor-not-allowed': 'cursor-pointer'} border-black border p-4 m-5`}>
